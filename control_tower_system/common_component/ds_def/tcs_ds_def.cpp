...
PointAndPad::PointAndPad(  neb::CJsonObject v_json){

    _point=Point(v_json["_point"]);
    _pad=Pad(v_json["_pad"]);
}
...
DirectCamOp::DirectCamOp(  neb::CJsonObject v_json):
    _camname(v_json("_camname"))
{

    _op=(PointAndPad(v_json["_op"]));
}
