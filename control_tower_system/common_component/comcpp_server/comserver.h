...
typedef struct  UserItem{
  ...
  bool operator<(const UserItem& other)  const
  ...
  UserItem& operator=(const UserItem& cls)
  {
      // 避免自赋值
      if (this != &cls)
      {
        _username=cls._username;
        _hash=cls._hash;
      }
  ...
  UserItem(const UserItem &v_it):_username(v_it._username),_hash(v_it._hash){}
