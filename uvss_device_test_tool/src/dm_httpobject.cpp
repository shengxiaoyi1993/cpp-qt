...
int dm_HTTPObject::Get(QString httpurl, QByteArray &response, int timeout) {
  ...
  //    qDebug()<<QByteArray::fromStdString(httpurl.toStdString()).toPercentEncoding("/:{},@.?");
...
int dm_HTTPObject::GetWithNoReturn(QString httpurl, QByteArray &response, int timeout){
  QNetworkAccessManager accessManager;
  QNetworkRequest request;
  QUrl url(httpurl);
  request.setUrl(url);
...
int dm_HTTPObject::ConnectToHTTPWithNoReturn(QString httpurl,QString &response){
  QByteArray buff;
  int status = GetWithNoReturn(httpurl, buff, 3);
  response = buff;
  return status;
}
